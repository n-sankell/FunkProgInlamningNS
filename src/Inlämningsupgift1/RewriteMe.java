...
public class RewriteMe {
    ...
    public int getAmountOfQuestionsInDatabase(){
        ...
        //return questions.stream().map(Question::getQuestionString).toList().size();
    ...
    public int getAmountOfQuestionsForACertainCategory(Category category){
        return questions.stream().filter(e -> e.getCategory() == category).toList().size();
    }
    ...
    public boolean isThisAnAnswerOption(String answerCandidate){
        return questions.stream().map(Question::getAllAnswers).anyMatch(e -> e.contains(answerCandidate));
    }
    ...
    public int getAnswerCandidateFrequncy(String answerCandidate){
        return questions.stream().map(Question::getAllAnswers).filter(e -> e.contains(answerCandidate)).toList().size();
    }
